function PinegrowVS() {
    ...
    const fs = require('fs');
    let configurations = vscode.workspace.getConfiguration("pinegrow");
    ...
    var debug = configurations.debug;
    ...
    var mapFuzzyUrlToPinegrowUrl = {};

    function getEditorUrl(document) {
        if(document.uri) {
            return document.uri.toString(true); //skip encoding
        }
    ...
    function getPgEditorUrl(document) {
        var url = getPgUrlFromEditorUrl(getEditorUrl(document));
        var fuzzy_url = getFuzzyUrl(url);
        if(mapFuzzyUrlToPinegrowUrl[fuzzy_url]) {
            return mapFuzzyUrlToPinegrowUrl[fuzzy_url];
        }
        return url;
    }
    ...
    function getFuzzyUrl(url) {
        var n = url.replace(/^file\:\/+/, ''); //remove file://
        n = n.replace(/:/g, '');
        n = n.replace(/\\/g, '/'); //replace all \ with /
        n = n.replace(/\/\/+/g, '/'); //replace multiple // with /
        n = n.toLowerCase();
        ...
        //var a = n.split('/');
        //a.shift(); //ignore the first item in path
        //a.join('/');
        return n;
    }
    ...
    function clearSavedInEditorInterval(force) {
        if (force || codeSavedInEditorInterval !== null) {
            clearInterval(codeSavedInEditorInterval);
    ...
    this.revertEditorFromDisk = function (editor) {
        ...
        const savedSource = fs.readFileSync(path);
        if (savedSource.toString() === editor.document.getText()) {
    ...
    this.isFileOpenInPinegrow = function (url) {
        var found = this.filesOpenInPinegrow.indexOf(url) >= 0;
        if(debug) {
            if(found) {
                console.log('Url is open in Pinegrow: ' + url)
            } else {
                console.log('Url is NOT open in Pinegrow: ' + url)
            }
        }
        return found;
    }
    ...
    function showErrorMessageWithSettingsButton(msg) {
        vscode.window.showErrorMessage(
            msg,
            'Open Settings'
        ).then(selectedAction => {
            if (selectedAction === 'Open Settings') {
                vscode.commands.executeCommand('workbench.action.openSettings', 'Pinegrow');
            }
        });
    }

    this.isEnabled = function(silent) {
        if(!configurations.enabled) {

            if(!silent) {
                vscode.window.showInformationMessage(
                    'Pinegrow Live Sync is not enabled for this workspace.',
                    'Open Settings'
                ).then(selectedAction => {
                    if (selectedAction === 'Open Settings') {
                        vscode.commands.executeCommand('workbench.action.openSettings', 'Pinegrow');
    ...
    this.init = function () {
        var _this = this;
        configurations = vscode.workspace.getConfiguration("pinegrow");
        pinegrow_url = configurations.urlWithPort;
        ...
        try {
            ...
            if (!urlinfo.hostname || !urlinfo.port) {
                showErrorMessageWithSettingsButton('Hostname and port are not set in Pinegrow URL.');
                return;
            }
        } catch (error) {
            showErrorMessageWithSettingsButton(`Invalid Pinegrow URL: "${url || ''}"`);
            return;
        }
        ...
        editor_api.on('error', function () {
            showErrorMessageWithSettingsButton("Unable to connect to Pinegrow at " + url + '.');
        });
        ...
        editor_api.on('codeSaved', function(data) {
            ...
            editors.forEach(function (editor) {
                ...
                var url = getPgEditorUrl(document);
                if (url == data.url) {
                    _this.revertEditorFromDisk(editor);
                }
            })
        });
        ...
        editor_api.on('listOfOpenFiles', function (data) {
            ...
            mapFuzzyUrlToPinegrowUrl = {};
            _this.filesOpenInPinegrow.forEach(function(url) {
                mapFuzzyUrlToPinegrowUrl[ getFuzzyUrl(url) ] = url;
            })

            if(debug) {
                console.log('Got list of open files in Pinegrow', data.list);
            }
        })
    }
